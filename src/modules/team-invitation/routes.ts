...
teamInvitationRoutes.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const [schemaError, parsedSchema, schemaCode] = validateSchema(
      teamInvitationInviteSchema,
      { ...data, teamId, invitedById: userId },
    );

    if (schemaError) return res.status(schemaCode).json({ error: schemaError });

    const invitation = await teamInvitationInvite({
      ...parsedSchema.data,
      invitedById: userId,
    });
...
teamInvitationRoutes.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!teamId || !invitationId)
      return res
        .status(400)
        .json({ error: "Team ID and Invitation ID are required." });

    const invitation = await teamInvitationResendEmail(
